...
- **Frontend tests: 258/324 passing (80%)** — 324 test cases written, MSW 2.x infrastructure fixed (custom Babel transform + polyfills needed for CRA 5.0.1). 66 tests need assertion rewrites to match actual component DOM.
- **Trade Builder root cause: user_id mismatch** — JWT contains different user_id than teams table. Diagnostic endpoint built (`/api/admin/diagnose-user/:email`). Needs Taylor's JWT to fix.
